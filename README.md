...
1. **Schema Definition**: The prompt takes in a schema that describes each column in the AE dataset, including labels, descriptions, and valid values

2. **Question Parsing**: Questions are sent to GPT-4o-mini model along with the schema, and the llm returns a json specifying which column to query and what value to filter for
